...
export class WorkplaceService extends BaseService {
    ...
    updateWorkplace = async (id: number,code: string, name: string, address: string, latitude: string, longitude: string): Promise<UpdateResult> => await this.workplaceRepo.update(WorkplaceEntity).set({  code, name, address, latitude, longitude }).where({ id }).execute()
