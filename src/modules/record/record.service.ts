...
export class RecordService extends BaseService {
    ...
    insertRecord = async (type: number, userId: number, workplaceId: number, isSameLocation: boolean, note: string): Promise<InsertResult> => await this.recordRepo.insert().into(RecordEntity).values({ type, userId, date: DateUtils.nowUtc(), workplaceId, isSameLocation, note }).execute()
